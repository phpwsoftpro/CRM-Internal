...
class MailingMailing(models.Model):
    ...
    test_subject = fields.Many2one("mailing.mailtext",string="Mail Template", ondelete="set null")

    @api.model
    def default_get(self, fields_list):
        vals = super().default_get(fields_list)
        default_template = self.env['mailing.mailtext'].search([], limit=1, order="id ASC")
        if default_template:
