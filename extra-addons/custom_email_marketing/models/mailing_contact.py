...
class MailingContact(models.Model):
    ...
    def _ensure_partner_links(self):
        for contact in self:
            ...
            company = self.env["res.company"].sudo().search([("domain", "=", domain)], limit=1)
            if not company:
            ...
            partner = self.env["res.partner"].sudo().search([("email", "=", contact.email)], limit=1)
    ...
    def send_email(self, subject, body):
        ...
        for contact in self:
            history_vals = {
                ...
                "company_id": contact.partner_id.company_id.id if contact.partner_id else False,
                "subject": subject,
                "body": body,
                "state": "sent",
            }
