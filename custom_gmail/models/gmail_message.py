...
class GmailMessage(models.Model):
    ...
    def fetch_gmail_messages(self, access_token):
        ...
        params = {
            'maxResults': 10  # Limit to the latest 10 emails
        }
        ...
        for msg in messages:
            ...
            if message_response.status_code == 200:
                ...
                subject = next(
                    (header.get('value') for header in headers_list if header.get('name') == 'Subject'), 'No Subject'
                )
                raw_date = next(
                    (header.get('value') for header in headers_list if header.get('name') == 'Date'), None
                )
